...
@pytest.mark.slow
def test_lookup_article_page_for_single_only_one_version(
    firefox_driver, marketwatch, core_tatsunoko
):
    with firefox_driver() as driver:
        marketwatch._lookup_article_page_for_single(driver, core_tatsunoko)
    expected = "https://www.cardmarket.com/en/YuGiOh/Products/Singles/Clash-of-Rebellions/Tatsunoko"
    assert core_tatsunoko.article_page == expected


@pytest.mark.slow
def test_lookup_article_page_for_single_duelist_league(
    firefox_driver, marketwatch, dl_krebons
):
    with firefox_driver() as driver:
        marketwatch._lookup_article_page_for_single(driver, dl_krebons)
    expected = "https://www.cardmarket.com/en/YuGiOh/Products/Singles/Duelist-League-09/Krebons-V2-Rare"
    assert dl_krebons.article_page == expected


@pytest.mark.skip("cannot implement correctly until I understand the logic")
def test_lookup_article_page_for_single_with_language_code(
    firefox_driver, marketwatch, lob_dark_magician
):
    with firefox_driver() as driver:
        marketwatch._lookup_article_page_for_single(driver, lob_dark_magician)
    ...
    assert lob_dark_magician.article_page == expected
