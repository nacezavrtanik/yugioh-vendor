...
@pytest.mark.parametrize(("enforced_type", "value"), [
    (str, "123"),
    (int, 123),
    (list, [123]),
    (tuple, (123,)),
])
def test_enforced_type_succeeds_for_correct_type(enforced_type, value):
    class C:
        attr = EnforcedType(enforced_type)
...
def test_enforced_type_raises_type_error_for_wrong_type(enforced_type, value):
    class C:
        attr = EnforcedType(enforced_type)
...
def test_enforced_type_succeeds_for_allow_none_is_true():
    class C:
        attr = EnforcedType(str, allow_none=True)

    instance = C()
    instance.attr = "asd"
    assert instance.attr == "asd"
    instance.attr = None
    assert instance.attr is None


def test_enforced_type_succeeds_for_allow_none_is_false():
    class C:
        attr = EnforcedType(str)
